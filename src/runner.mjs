...
2. HYPOTHESIS: ONE concrete, testable heuristic that would fix that failure (a direct URL shortcut, a specific wait, an exact selector/sequence, a success check). IMPORTANT: if the executor burned many turns fighting a UI widget (date picker, dropdown, modal, autocomplete), do NOT propose a better way to click it — propose ELIMINATING it. Most sites encode the full query in the URL. The most reliable pattern: a natural-language search query appended as "?q=" with the value URL-encoded (spaces = %20) — e.g. "https://www.google.com/travel/flights?q=nonstop%20one-way%20flights%20from%20SFO%20to%20JFK%20on%202026-07-15" lands directly on the results list. STRONGLY prefer this "?q=" natural-language form over hash anchors (#flt=), protobuf "tfs=" params, or any guessed param syntax — those are unreliable. Put the full intent (route, date, one-way, nonstop) into the natural-language query itself.
